...
//AI Machine Learning Primary
{
	...
	public void Set_FileSave(string s_Link)
	{
		...
		myFile.Set_Data_Write_Add(this.i_LayerCount);

		myFile.Set_Data_Write_Add("NeuralCount:");
		for (int lay = 0; lay < i_LayerCount; lay++)
		{
			myFile.Set_Data_Write_Add(this.li_NeuralCount[lay]);
		...
		for (int lay = 0; lay < i_LayerCount - 1; lay++)
		{
			myFile.Set_Data_Write_Add(this.li_Bias[lay]);
			//Lưu Bias of  Layer
		}
		...
		for (int lay = 0; lay < i_LayerCount - 1; lay++)
		{
			...
			for (int neuY = 0; neuY < li_NeuralCount[lay + 1]; neuY++)
			{
				...
				for (int neuX = 0; neuX < li_NeuralCount[lay]; neuX++)
				{
					...
					myFile.Set_Data_Write_Add(this.li3_Weight[lay][neuY][neuX]);
					//Lưu Weight of  Layer
				}
			}
		}
